...
class SapiensPoseEstimation:
    ...
    def __call__(self, img: np.ndarray, allowed_keypoints: set[str] | None = None) -> np.ndarray:
        ...
        pose_result_image, keypoints = self.estimate_pose(img, bboxes, allowed_keypoints)
    ...
    def estimate_pose(
        ...
        for bbox in bboxes:
            ...
            keypoints_rel = self.heatmaps_to_keypoints(heatmaps[0].cpu().numpy())  # heatmap座標 (x,y in 192x256)
            keypoints_rel = self._filter_keypoints(keypoints_rel, allowed_keypoints)

            # heatmap座標 -> 元画像座標へ変換
            x1, y1, x2, y2 = map(int, bbox[:4])
            bbox_w, bbox_h = x2 - x1, y2 - y1
            keypoints_abs = {}
            for name, (x, y, conf) in keypoints_rel.items():
                abs_x = float(x) * bbox_w / 192 + x1
                abs_y = float(y) * bbox_h / 256 + y1
                keypoints_abs[name] = (abs_x, abs_y, conf)

            all_keypoints.append(keypoints_abs)

            # Draw the keypoints on the original image
            result_img = self.draw_keypoints(result_img, keypoints_abs, bbox)
    ...
    @staticmethod
    def _filter_keypoints(keypoints: dict, allowed_keypoints: set[str] | None) -> dict:
        if allowed_keypoints is None:
            return keypoints
        return {name: coords for name, coords in keypoints.items() if name in allowed_keypoints}
