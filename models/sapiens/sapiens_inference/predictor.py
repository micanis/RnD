...
class SapiensPredictor:
    def __init__(self, config: SapiensConfig):
        ...
        self.pose_predictor = SapiensPoseEstimation(config.pose_type, config.device, config.dtype) if self.has_pose else None
    ...
    def predict(self, img: np.ndarray) -> np.ndarray:
        ...
        # Pose estimation on full image
        if self.has_pose:
            pose_img, keypoints = self.pose_predictor(img)
            pose_results = [pose_img, keypoints]

        return self.draw_maps(img, person_boxes, normal_maps, segmentation_maps, depth_maps, pose_results)

    # TODO: Clean this up
    def draw_maps(self, img, person_boxes, normal_maps, segmentation_maps, depth_maps, pose_results=[]):
        draw_img = []
        ...
        if self.has_pose and len(pose_results) > 0:
            draw_img.append(pose_results[0])  # pose_img
